...
import static com.tempus_indicium.datafilter.App.measurementsList;
...
//        try {
//            FileStore.dataOutputStream.close();
//            FileStore.dataOutputStream = new DataOutputStream(dataGatherSocket.getOutputStream());
//        } catch (IOException e) {
//            e.printStackTrace();
//        }
//    }
...
//    public static synchronized void writeToOutputStream() {
//        if (App.measurementBytes.remaining() == 0) {
//            System.out.println("PERFORMING WRITE");
//            try {
//                FileStore.dataOutputStream.write(App.measurementBytes.array());
//                FileStore.dataOutputStream.flush(); // zou hier nog wat kunnen zijn
//            } catch (IOException e) {
//                e.printStackTrace();
//            } finally {
...
//    }
    ...
    public static synchronized void writeToOutputStreamIfNeeded() {
        if (App.measurementsList.size() >= Integer.parseInt(App.config.getProperty("ROWS_PER_WRITE"))) {
            WorkerThread.isWriting = true;
            List<byte[]> tempList = App.measurementsList.subList(0, App.measurementsList.size());
            System.out.println("PERFORMING WRITE MEASUREMENTS: "+tempList.size());
            tempList.forEach(m -> {
                try {
